...
function fill_car_driver()
{
    ...
    foreach ($select as $row) {
        $output .= '<option value="' . $row["car_id"] . '">' . $row["car_license_plate"] . '</option>';
    }
...
function insert_update_delete()
{
    ...
    if (isset($_POST['btnupdate'])) {
        ...
        if (empty($category)) {
            ...
            $update = query("UPDATE tbl_subject set sj_name='$category', sj_price='$price',sj_price_year='$txtprice_year',car_price_year='$carprice_year',car_price_month='$carprice_month' where sj_id=" . $id);
            confirm($update);
...
function insert_update_delete_studytime()
{
    if (isset($_POST['btnsave'])) {
        ...
        if (empty($category)) {
            ...
            $insert = query("INSERT into tbl_studytime (sdi_name) values('{$category}')");
            confirm($insert);
            if ($insert) {
                set_message(' <script>
                Swal.fire({
    ...
    if (isset($_POST['btnupdate'])) {
        ...
        if (empty($category)) {
            ...
            $update = query("UPDATE tbl_studytime set sdi_name='$category' where sdi_id=" . $id);
            confirm($update);
    ...
    if (isset($_POST['btnedit'])) {
        ...
        if ($select) {
            ...
            echo '<div class="col-md-4">
               ...
               <input type="hidden" class="form-control" placeholder="Enter Category"  value="' . $row->sdi_id . '" name="txtcatid" >

               <input type="text" class="form-control" placeholder="Enter Category"  value="' . $row->sdi_name     . '" name="txtcategory" >
               </div>
...
function insert_update_delete_Classroom()
{
    if (isset($_POST['btnsave'])) {
        ...
        if (empty($category)) {
            ...
            $insert = query("INSERT into tbl_classroom (cr_name) values('{$category}')");
            confirm($insert);
            if ($insert) {
                set_message(' <script>
                Swal.fire({
