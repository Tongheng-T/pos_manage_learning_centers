            ...
            <div class="card-body">
              <div class="row">
                ...
                <div class="col-md-2">
                  ...
                  <button  class="btn btn-primary" id="print-btnre_repot">Print</button>
              ...
              $selectf = query("SELECT sd_sex, count(sd_sex) as totalF FROM tbl_students  where sd_sex= 'ស្រី'  ");
              confirm($selectt);
              $row = $selectt->fetch_assoc();

              $dd = $row['total'];

              $row = $selectf->fetch_assoc();

              $ddF = $row['totalF'];

              $select = query("SELECT sum(money) as subtotal , count(sdpay_id ) as invoice  from tbl_employee_students where date between '$date_1' AND '$date_2' ");
              confirm($select);
